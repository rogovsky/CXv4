...
static int  DoWrite(cda_d_dircn_t  var,
                    void          *privptr,
                    void          *data,
                    cxdtype_t      dtype,
                    int            n_items)
...
fprintf(stderr, "%s\n", __FUNCTION__);
    ...
    cda_d_dircn_chan_n_props(var, NULL, NULL, NULL, &addr);
    *((double*)addr) = v;
    SetSimpleKnobValue(k, v);

    cda_d_dircn_update_chan(var, n_items, 0);

    return 0;
}
static void DircnKCB(DataKnob k, double v, void *privptr)
{
  ...
  void *addr;
    ...
    cda_d_dircn_chan_n_props(var, NULL, NULL, NULL, &addr);
    *((double*)addr) = v;
    cda_d_dircn_update_chan(var, 1, 0);
    cda_d_dircn_update_cycle();
}
static void CreateDircnWindow(void)
{
  XhWindow      lwin;
  CxWidget      lgrid;
  int           n;
  ...
  DataKnob      k;
    ...
    lwin = XhCreateWindow("List of dircn vars", "zzz", "qqq",
    ...
    for (n = 0, count = 0;  n < cda_d_dircn_chans_count();  n++)
        if (cda_d_dircn_chan_n_props(n, &name, &dtype, NULL, &addr) >= 0  &&
...
fprintf(stderr, "%s: %p\n", name, addr);
            ...
            cda_d_dircn_override_wr(n, DoWrite, k);
...
{
  const char   *app_name  = "dircntest";
  const char   *app_class = "DircnTest";
  char         *p;
  int           arg_n;
  char         *sysname;
  DataSubsys    ds;
  char         *defserver;
    ...
    setenv("CDA_D_DIRCN_AUTO_CREATE", "1", 1);
    ...
    /* Create a "list of dircn vars used" window */
    CreateDircnWindow();
