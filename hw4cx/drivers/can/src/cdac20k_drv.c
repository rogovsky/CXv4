...
typedef struct
{
    ...
    int32  act;    // AcCeleration Time (in heartbeats)
...
static int DoCalcMovement(int32 trg, cankoz_out_ch_t *ci, cankoz_out_ch_t *rp)
{
  int32  mxs;
  int32  a1;
  int32  a2;
  ...
  int32  spd;
    ...
    if (ci->spd == 0  ||  ci->act <= 0) return DO_CALC_R_SET_IMMED;

    mxs = abs(ci->spd) / HEARTBEAT_FREQ;
    a1 = mxs / ci->act;  if (a1 == 0) a1 = 1;
    ...
    if (DO_DBG) fprintf(stderr, "distance=%d d_half=%d pos=%d\n", distance, d_half, pos);
    rp->mxs = mxs;
    rp->crs = a2;
    rp->mns = a2;
    rp->acs = a1;
...
static psp_paramdescr_t cdac20_params[] =
{
    ...
    PSP_P_INT ("tac",  privrec_t, out[0].act, 0, 0, 1000),
    PSP_P_END()
};
        ...
        for (l = 0;  l < countof(me->out);  l++)
            if (me->out[l].isc  &&  me->out[l].nxt)
            {
                if (1 && me->out[l].act != 0)
                {
                    ...
                    {
                    if (DO_DBG) fprintf(stderr, "-@-%7d %7d -%-7d", abs(me->out[l].trg - val), me->out[l].crs, me->out[l].acs);
                        me->out[l].crs -= me->out[l].acs;
                        if (me->out[l].crs < me->out[l].mns)
                            me->out[l].crs = me->out[l].mns;
                    if (DO_DBG) fprintf(stderr, " ->%d\n", me->out[l].crs);
                    ReturnInt32Datum(devid, CHAN_CURSPD,  me->out[l].crs, 0);
                    ReturnInt32Datum(devid, CHAN_CURACC, -me->out[l].acs, 0);
                    }
    ...
    for (n = 0;  n < count;  n++)
    {
                 ...
                 chn <  KOZDEV_CHAN_OUT_n_base      + CDAC20_CHAN_OUT_n_count)
        {
            ...
            if (action == DRVA_WRITE)
            {
                ...
                {
                  cankoz_out_ch_t ccr;
                    DoCalcMovement(val, me->out + l, &ccr);
...
DEFINE_CXSD_DRIVER(cdac20k, "CDAC20/CEAC51/CEAC51A CAN-DAC/ADC driver",
                   NULL, NULL,
