...
static volatile unsigned *reg0, *reg1, *reg2, *vme_chip;

static spinlock_t vmelock = SPIN_LOCK_UNLOCKED;

vme_device_t IRQHandlers[8][255];

int insert_interrupt_handler(int irq, int irq_vect, fInterruptHandler irq_handler, void* device)
{
    if(irq & 0xfffffff8)
    {
        DP(printk("IRQ should be less or equal to 7\n"));
	return 1;
    }
    
    if(irq_vect & 0xffffff00)
    {
        DP(printk("IRQ vector be less or equal to 255\n"));
	return 1;
    }

    if(IRQHandlers[irq][irq_vect].handler != 0)
    {
        DP(printk("IRQ %d, VECTOR %d is busy\n", irq, irq_vect));
	return 1;
    }

    MOD_INC_USE_COUNT;
    DP(printk("Installing IRQ %d, VECTOR %d\n", irq, irq_vect));
    IRQHandlers[irq][irq_vect].handler = irq_handler;
    IRQHandlers[irq][irq_vect].device = device;
...
int remove_interrupt_handler(int irq, int irq_vect, void* device)
{
    ...
    if(IRQHandlers[irq][irq_vect].device != NULL && device != IRQHandlers[irq][irq_vect].device)
    {
        DP(printk("Device address 0x%p\n", device));    
        DP(printk("This handler was installed by other device\n"));
	return 1;
    }

    if(IRQHandlers[irq][irq_vect].handler != 0)
    {
	DP(printk("Removing IRQ %d, VECTOR %d\n", irq, irq_vect));
	
        MOD_DEC_USE_COUNT;
	IRQHandlers[irq][irq_vect].handler = NULL;
	IRQHandlers[irq][irq_vect].device = NULL;
    }
...
{
    unsigned long flags;
    unsigned int cur_mask, active_mask; 
    volatile unsigned int *inth;
    unsigned int irql;
    unsigned int value;
...
//    DP(printk("active mask 0x%x\n", active_mask));
	...
	*reg1 &= ~active_mask;
	...
	if (active_mask != 0x01) 
	{
	    ...
	    else if(vd[irql].vector < 256 && vd[irql].vector >= 0 && IRQHandlers[irql][vd[irql].vector].handler != 0)
	    {
		IRQHandlers[irql][vd[irql].vector].handler(irql, dev_id, regs, vd[irql].vector, IRQHandlers[irql][vd[irql].vector].device);
	    }
	}
	else 
	{
...
int init_module(void)
{
    ...
    memset(IRQHandlers, 0, sizeof(vme_device_t)*8*255);
...
//    printk("BIVME2 driver, V.R.Mamkin, 2005, release 1.1\n");
    printk("BIVME2 driver, V.R.Mamkin, D.S.Shichkov, 2011, release 1.2\n");
    return 0;
}
